...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    header("Location: ../public/exercice8.php?error=bad-method");
    exit();
}
...
if (!isset($_POST["prenom"]) || !isset($_POST["nom"]) || !isset($_POST["civilite"]) || !isset($_FILES["fichier"])) {
    header("Location: ../public/exercice8.php?error=missing-input");
    exit();
}
...
if (empty($_POST["prenom"]) || empty($_POST["nom"]) || empty($_POST["civilite"]) || empty($_FILES["fichier"])) {
    header("Location: ../public/exercice8.php?error=missing-value");
    exit();
}
...
$fichier=  htmlspecialchars(trim($_FILES["fichier"]));

// On vérifie que les règles d'usages sont respectées
if (strlen($prenom) > 50) {
    header("Location: ../public/exercice8.php?error=max-length");
    exit();
}
